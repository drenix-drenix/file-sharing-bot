...
async def start_command(message: types.Message, state: FSMContext):
    ...
    if db.get_users_exist(message.chat.id) == False:
        ...
        if not args:
            await bot.send_message(chat_id=message.chat.id, text='👋 Привет, я бот, который поможет тебе поделиться файлами с другими пользователями.\n\n 👤 Поддержка – @drenix_x', reply_markup = kb.menu_kb())
        else:
            type_file, fileID, views, password = db.get_file(args)
            if type_file is None and fileID is None:
                await bot.send_message(chat_id=message.chat.id, text='⚠️ Файл не найден!', reply_markup = kb.menu_kb())
            else:
                if password == (None,):
        ...
        if not args:
            await bot.send_message(chat_id=message.chat.id, text='👋 Привет, я бот, который поможет тебе поделиться файлами с другими пользователями.', reply_markup = kb.menu_kb())
        else:
            type_file, fileID, views, password = db.get_file(args)
            if type_file is None and fileID is None:
                await bot.send_message(chat_id=message.chat.id, text='⚠️ Файл не найден!', reply_markup = kb.menu_kb())
...
async def upload_file(message: types.Message, state: FSMContext):
    ...
    if message.text:
        if message.text.lower() == '❌ отмена':
            await bot.send_message(chat_id=message.chat.id, text='↩️ Вы вернулись в главное меню.', reply_markup=kb.menu_kb())
            await state.finish()
        else:
            ...
            if message.text == password[0]:
                ...
                await bot.send_message(chat_id=message.chat.id, text='🛡️ Упс, это не верный пароль, попробуй еще раз:', reply_markup = kb.back_kb())
    else:
        await bot.send_message(chat_id=message.chat.id, text='🛡️ Упс, это не верный пароль, попробуй еще раз:', reply_markup = kb.back_kb())
...
async def create_post(message: types.Message):
    if db.get_users_exist(message.chat.id) == True:
        ...
        if all_types == []:
            await bot.send_message(message.chat.id, text='⚠️ У вас нету загруженных файлов, чтобы загрузить файлы нажмите "📤 Загрузить файл"', reply_markup = kb.menu_kb())
        else:
            text='📁 Ваши файлы: \n\n'
...
async def upload_file(message: types.Message, state: FSMContext):
    ...
    if message.photo:
        ...
        if message.text.lower() == '❌ отмена':
            await bot.send_message(chat_id=message.chat.id, text='↩️ Ты вернулся назад.', reply_markup=kb.menu_kb())
            await state.finish()
        else:
...
async def handler_call(call: types.CallbackQuery, state: FSMContext):
    ...
    if call.data == 'delete_file':
        ...
        if all_ids == []:
            ...
            await bot.send_message(chat_id=chat_id, text='⚠️ У вас нету загруженных файлов, чтобы загрузить файлы нажмите "📤 Загрузить файл"', reply_markup = kb.menu_kb())
        else:
            text='🗑️ Файлы для удаления: \n\n'
            for i, id_file in enumerate(all_ids):
                text+=f'{i+1}. https://t.me/{str(bot_name)}?start={id_file[0]} | {all_types[i][0]} | {all_views[i][0]} | 🔑 Пароль: {passwords[i][0]}\n\n'
